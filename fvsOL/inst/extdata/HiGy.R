...
ht.pred.parm = dplyr::tribble(
  ...
  'base',  'AK',      31.8661,  0,      0.0185562,  0.801602,  0.0652061,  -0.329508,  # base form without BYI, refit 2026-09-16, rDBH = DBH/DBH.max
  'site',  'AK',      30.1882,  1.42629,  0.0184009,  0.817994,  0.0511083,  -0.35086)  # manuscript Table 3, refit 2026-09-16, rDBH = DBH/DBH.max
...
calc_ht = function(tree.data, plot.data, byi=stand$byi, 
  ...
  tree = tree.data %>% 
    dplyr::group_by(plot) %>% dplyr::mutate(dbh.max = max(dbh, na.rm = TRUE)) %>% dplyr::ungroup() %>%  # largest live DBH per plot
